...
const Product = ({ productSlug }) => {
  ...
  const addToCart = (product, quantity) => {
    ...
    const existingProduct = cart.find((item) => item.product[0].id === product[0].id);
...
<div className="div1">       <img className="prodImg"src={product[0].images[0].src} alt={product[0].name} /></div>
<div className="div2">    <h3 className="productTitle">{product[0].name}</h3>
<div>
...
<div className="product">
    ...
    <div className="productDescription"dangerouslySetInnerHTML={{ __html: product[0].short_description }} />
...
<div className="number">
  ...
  <span className="plus" onClick={incrementQuantity}>+</span>
    ...
    <div className="add">
    <button className="addButton" onClick={() => addToCart(product, quantity)}>Add to cart</button>
